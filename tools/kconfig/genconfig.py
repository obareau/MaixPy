...
kconfig_lib_path = f"{sys.path[0]}/Kconfiglib"
...
def _cmake_contents(kconfig, header):
    ...
    for sym in kconfig.unique_defined_syms:
        ...
        if sym.orig_type in (kconfiglib.BOOL, kconfiglib.TRISTATE) and val == "n":
            val = ""
        add(f'set({kconfig.config_prefix}{sym.name} \"{val}\")\n')
        config_vars.append(str(kconfig.config_prefix+sym.name))
    add(f'set(CONFIGS_LIST {";".join(config_vars)})\n')
    return "".join(chunks)


def write_config(kconfig, filename, gui):
    print(f"-- Write makefile config at: {filename}")
    if not gui:
        kconfig.write_config(filename)

def write_cmake(kconfig, filename, gui):
    print(f"-- Write  cmake  config  at: {filename}")
    cmake_conf_header = (
...
def write_header(kconfig, filename, gui):
    print(f"-- write  c header file  at: {filename}")
...
# write back
for filename in out_format.values():
